...
PARAM  <- list()
PARAM$experimento <- "FINAL_V4_FE9000"

PARAM$exp_input  <- "FINAL_V4_DR9000"
...
PARAM$CanaritosAsesinos  <- T
# FIN Parametros del script

meses= c( 201908, 201909, 201910,  202008, 202009, 202101, 202102, 202103, 202104, 202105, 202106, 202103, 202104 )
mes_valid = 202105
...
AgregaVarRandomForest  <- function( num.trees, max.depth, min.node.size, mtry, meses)
{
  gc()
  ...
  for( arbol in 1:num.trees )
  {
    ...
    for( pos in 1:length(hojas_arbol) )
    {
      ...
      dataset[  ,  paste0( "rf_", sprintf( "%03d", arbol ), "_", sprintf( "%03d", nodo_id ) , '1st') := 0L ]
      
      dataset[ which( rfhojas$predictions[ , arbol] == nodo_id ,  ), 
               paste0( "rf_", sprintf( "%03d", arbol ), "_", sprintf( "%03d", nodo_id ) , '1st' ) := 1L ]
    }
  }
...
CanaritosAsesinos  <- function( canaritos_ratio=0.2, no_sacar=c() ,meses)
{
  gc()
  dataset[ , clase01:= ifelse( clase_ternaria=="CONTINUA", 0, 1 ) ]
  ...
  dataset[ , entrenamiento := foto_mes %in% meses  & ( clase01==1 | azar < 0.10 ) ]
